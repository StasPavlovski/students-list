 ...
 * @returns {{ name: string, surname: string, midname: string, birthDate: string, studyYear: string, faculty: string }} Объект студента
 */
function makeStudentFromData(data) {
  ...
  const student = {
    ...
    midname: asString(data.midname),
    birthDate: asString(data.birthDate),
    studyYear: asString(data.studyYear),
  ...
  if (!student.midname) errors.push({ field: 'midname', message: 'Не указано отчество' });

  if (!student.birthDate) errors.push({ field: 'birthDate', message: 'Не указана дата рождения' });
  if (!student.studyYear) errors.push({ field: 'studyYear', message: 'Не указано начало обучения' });
 ...
 * @returns {{ id: string, name: string, surname: string, midname: string, birthDate: string, studyYear: string, faculty: string }[]} Массив студентов
 */
 ...
 * @returns {{ id: string, name: string, surname: string, midname: string, birthDate: string, studyYear: string, faculty: string, createdAt: string, updatedAt: string }} Объект студента
 */
 ...
 * @param {{ name?: string, surname?: string, midname?: string, birthDate?: string, studyYear?: string, faculty?: string }} data - Объект с изменяемыми данными
 * @throws {ApiError} Студент с таким ID не найден (statusCode 404)
 * @throws {ApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ id: string, name: string, surname: string, midname: string, birthDate: string, studyYear: string, faculty: string, createdAt: string, updatedAt: string }} Объект студента
 */
...
module.exports = createServer(async (req, res) => {
  ...
  .on('listening', () => {
    if (process.env.NODE_ENV !== 'test') {
      ...
      console.log(`POST ${URI_PREFIX} - создать студента, в теле запроса нужно передать объект { name: string, surname: string, midname: string, birthDate: string, studyYear: string, faculty: string}`);
      console.log(`GET ${URI_PREFIX}/{id} - получить студента по его ID`);
      console.log(`PATCH ${URI_PREFIX}/{id} - изменить студента с ID, в теле запроса нужно передать объект { name?: string, surname?: string, midname?: string, birthDate?: string, studyYear?: string, faculty?: string}`);
